...
**✅ DONE (visual; audio TODO in the 80s batch). One-and-done splat — NO re-roll, NO re-splat (2026-06-08).** Gemini snapped it FIRST roll (own clean chat). Unlike Canberra, Gemini gave the prompted **flat end wall**: a tall blank pale bay flanked by symmetric side galleries of small works, deep one-point recession down a polished travertine floor, clerestory glazing overhead, and — the period payoff — the gleaming new Minato Mirai skyline (Landmark Tower + harbor) out the SIDE windows, exactly the "brand-new waterfront" read (a strict purist would flag Landmark Tower as ~1993, but it's a distant side-window detail and the rule-of-cool vibe is dead-on; non-blocking). All four gates passed on the 2D still: **aspect ≈5:8** portrait, deep recession to the end wall, **flat slim dark frame** (no shadowbox), **dark-on-pale contrast** (the harbor windows stayed to the SIDE, no glare washing out the frame edge). The contrast paid off: splat opening came back ≈**0.71** — close enough to the 0.685 scan that, like Canberra, **NO crop/aspect override needed**. ✦ stripped via a same-row Poisson floor-copy patch (grain + grout continuity preserved, not a smudgy inpaint) → `Splats/Refs/yokohama-1989_clean.png`. **Calibration (the depth lesson, most extreme yet):** head-on triangulation (140px@z0 / 258px@z−6) AND the head-on parallax-null BOTH read the frame plane at ≈z−11 — but the **OBLIQUE view showed the painting floating LEFT of the frame**: head-on UNDER-READ depth by **~2m** (vs Bordeaux ~1m, Canberra ~0.4m — the deepest hall, the biggest error). Re-fit from oblique registration, confirmed from BOTH left and right oblique → true frame plane **z≈−13**. Frame is FLAT so the canvas seats **FLUSH, no float, no wash-out** (verified razor-crisp at 4m close head-on, signature legible). Calibrated: `WORLD_PAINTING` **[−0.018,1.176,−13] h1.564** (default url+aspect; over-cover onto the dark lip, height bumped + y lowered to close faint bare-canvas slivers top/right/bottom — invisible on the dark frame) · `WORLD_SPAWN` **[−0.018,1.6,0]** (room-origin axial establishing hero — the vast skylit hall IS the subject, like met-74/stockholm; camera-x aligned to frame so the painting projects dead-center) · **NO placard** (Yokohama isn't a label world). Cold-reload via the real path verified. Source `Splats/Refs/yokohama-1989_clean.png`.
